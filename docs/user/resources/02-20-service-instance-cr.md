...
| services.cloud.sap.com/allowCrossNamespaceBinding | string | Set to `"true"` to allow `ServiceBinding` resources from other namespaces to reference this instance. Required for cross-namespace bindings. |
| services.cloud.sap.com/allowedNamespacesForBinding | string | Comma-separated list of namespaces permitted to create bindings to this instance. Only evaluated when `allowCrossNamespaceBinding` is `"true"`. If omitted, all namespaces are allowed. |