...
| **serviceInstanceNamespace** | string | The namespace of the service instance to bind; if not specified, the default is the binding's namespace. For cross-namespace bindings, the referenced `ServiceInstance` must have the annotation `services.cloud.sap.com/allowCrossNamespaceBinding: "true"`. If the annotation is missing or the binding's namespace is not in the allowlist, the binding is blocked: the `Succeeded` condition is set to `false` with reason `Blocked`. Note: the blocked error message is identical to the one shown when the instance does not exist. |
