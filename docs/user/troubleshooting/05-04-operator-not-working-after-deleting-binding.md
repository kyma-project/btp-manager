...
Alternatively, if the `sap-btp-manager` Secret itself was deleted from the cluster, Kyma's automatic reconciliation restores it within 24 hours. In that case, no support ticket is needed — wait for the Secret to be restored and the SAP BTP Operator module to recover.
...
3. In that Service Manager instance, check whether a binding exists.
   - If no binding exists, re-create it.
   - If a binding already exists (whether original or re-created), proceed to the next step.

4. Check the binding's creation date and compare it with the Service Manager instance's creation date. If the binding was created later, it was deleted and re-created — this confirms you're in the scenario described by this guide.

   > [!NOTE]
   > If the dates match, the binding is original and hasn't been re-created. In that case, the `sap-btp-manager` Secret may have been deleted directly from the cluster. Kyma's automatic reconciliation restores it within 24 hours — no support ticket is needed. If you can't wait, you can restore the Secret immediately by following [Customize the Default Credentials and Access](../03-11-customize_secret.md) using the credentials from the existing binding.
