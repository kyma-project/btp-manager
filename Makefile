...
module-build: kyma kustomize ## Build the Module and push it to a registry defined in MODULE_REGISTRY
	cd config/manager && $(KUSTOMIZE) edit set image controller=${IMG}
	$(KYMA) alpha create module --channel=${MODULE_CHANNEL} --name kyma.project.io/module/$(MODULE_NAME) --version $(MODULE_VERSION) --target="remote" --output="template.yaml" --path . $(MODULE_CREATION_FLAGS)
	$(KYMA) alpha create module --channel=${MODULE_CHANNEL} --name kyma.project.io/module/$(MODULE_NAME) --version $(MODULE_VERSION) --target="control-plane" --output="template_control_plane.yaml" --path . $(MODULE_CREATION_FLAGS)
