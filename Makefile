...
module-build: kyma kustomize ## Build the Module and push it to a registry defined in MODULE_REGISTRY
	cd config/manager && $(KUSTOMIZE) edit set image controller=${IMG}
#	$(KYMA) alpha create module $(SECURITY_SCAN_OPTIONS) --name kyma.project.io/module/$(MODULE_NAME) --version $(MODULE_VERSION) --path . $(MODULE_CREATION_FLAGS)
	$(KYMA) alpha create module $(SECURITY_SCAN_OPTIONS) --version $(MODULE_VERSION) --path . $(MODULE_CREATION_FLAGS)
