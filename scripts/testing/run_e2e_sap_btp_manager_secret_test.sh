...
kubectl patch secret -n ${NAMESPACE} ${BTP_MANAGER_SECRET_NAME} -p "{\"data\":{\"clientid\":\"${CLIENT_ID}\",\"clientsecret\":\"${CLIENT_SECRET}\",\"sm_url\":\"${SM_URL}\",\"tokenurl\":\"${TOKEN_URL}\",\"cluster_id\":\"${ENCODED_CLUSTER_ID}\",\"management_namespace\":\"${ENCODED_MANAGEMENT_NAMESPACE}\"}}"
...
do
  ...
  if ! ${SAP_BTP_OPERATOR_SECRET_RESOURCE_VERSION_CHANGED}; then
    ...
    if [[ "${ACTUAL_SAP_BTP_OPERATOR_SECRET_RESOURCE_VERSION}" != "${SAP_BTP_OPERATOR_SECRET_RESOURCE_VERSION}" ]]; then
      echo "${SAP_BTP_OPERATOR_SECRET_NAME} secret version changed"
  ...
  if ! ${SAP_BTP_OPERATOR_CLUSTER_ID_SECRET_ID_CHANGED}; then
    if [[ "$(kubectl get secret -n ${NAMESPACE} ${SAP_BTP_OPERATOR_CLUSTER_ID_SECRET_NAME} 2>&1)" != *"Error from server (NotFound)"* ]]; then
      ...
      echo "${SAP_BTP_OPERATOR_CLUSTER_ID_SECRET_NAME} secret does not exist in ${NAMESPACE} namespace"
    fi
  fi
